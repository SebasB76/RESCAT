  ...
  return (
    <Link href={`/?box=${box.id}`} scroll={false} className="group flex min-w-0 flex-col rounded-xl bg-white p-2 ring-1 ring-pino/12 transition-[transform,box-shadow] duration-200 hover:-translate-y-0.5 hover:ring-pino/25 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-hoja">
      ...
      <div className="flex flex-1 flex-col px-1 pb-1 pt-3">
        ...
        <div className="mt-auto flex items-end justify-between gap-3 border-t border-pino/10 pt-3"><div><p className="text-xs text-pino/70 line-through tabular-nums">{money(box.originalPrice)}</p><p className="text-2xl font-black leading-none tracking-[-0.03em] text-pino tabular-nums">{money(box.price)}</p></div><div className="text-right">{saved > 0 && <p className="text-xs font-bold text-hoja">Ahorras {money(saved)}</p>}<p className="mt-1 text-[0.68rem] text-pino/70">Salvas ~{foodKgSaved(box.tipo)} kg</p></div></div>
      </div>
    </Link>
  )
}
