...
function Stat({ value, label }: { value: string; label: string }) {
  return (
    <div>
      <div className="font-display text-3xl leading-none text-dorado tabular-nums">{value}</div>
...
export function HeroStats({ boxesCount, productCount, avgSavingsPct }: HeroStatsProps) {
  return (
    <section className="relative overflow-hidden rounded-3xl bg-pino px-6 py-10 text-cream sm:px-10">
      <div className="pointer-events-none absolute -top-24 -right-16 h-72 w-72 rounded-full bg-hoja/20 blur-3xl" />
      <div className="relative">
        ...
        <div className="mt-6">
          <a href="#catalogo" className="inline-flex items-center gap-1.5 rounded-full bg-dorado px-4 py-2 text-sm font-semibold text-pino transition hover:bg-dorado/90">
